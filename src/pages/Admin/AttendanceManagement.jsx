...
export default function AttendanceManagement() {
  const [records, setRecords] = useState([
    { id: 1, name: 'John Doe', role: 'Student', date: '2026-07-12', status: 'Present', class: '10-A' },
    { id: 2, name: 'Jane Smith', role: 'Teacher', date: '2026-07-12', status: 'Absent', class: 'N/A' },
    { id: 3, name: 'Alice Johnson', role: 'Student', date: '2026-07-11', status: 'Late', class: '9-B' },
    { id: 4, name: 'Bob Brown', role: 'Student', date: '2026-07-12', status: 'Present', class: '10-A' },
    { id: 5, name: 'Charlie Davis', role: 'Teacher', date: '2026-07-12', status: 'Present', class: 'N/A' },
    { id: 6, name: 'Diana Prince', role: 'Student', date: '2026-07-11', status: 'Present', class: '9-B' },
    { id: 7, name: 'Eve Adams', role: 'Student', date: '2026-07-12', status: 'Absent', class: '10-A' },
    { id: 8, name: 'Frank Castle', role: 'Teacher', date: '2026-07-12', status: 'Late', class: 'N/A' },
    { id: 9, name: 'Grace Hopper', role: 'Student', date: '2026-07-11', status: 'Present', class: '9-B' },
    { id: 10, name: 'Hank Pym', role: 'Student', date: '2026-07-12', status: 'Present', class: '10-A' },
  ]);
  const [showModal, setShowModal] = useState(false);
  const [confirmModalState, setConfirmModalState] = useState({ isOpen: false, idToDelete: null });
  const [formData, setFormData] = useState({ name: '', role: 'Student', date: new Date().toISOString().split('T')[0], status: 'Present', class: '' });
  ...
  const handleSave = (e) => {
    ...
    setShowModal(false);
    setFormData({ name: '', role: 'Student', date: new Date().toISOString().split('T')[0], status: 'Present', class: '' });
  };
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto h-[calc(100vh-2rem)] flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 gap-4 shrink-0">
        <div>
          ...
          <p className="text-slate-500 mt-1">Track and manage student and staff attendance daily.</p>
        </div>
        <button 
          onClick={() => { setFormData({ name: '', role: 'Student', date: new Date().toISOString().split('T')[0], status: 'Present', class: '' }); setShowModal(true); }}
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                ...
                <th className="p-4 font-semibold text-slate-600 border-b border-slate-200">Role & Class</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="text-sm font-medium text-slate-700">{record.role}</div>
                    {record.class && record.class !== 'N/A' && <div className="text-xs text-slate-500">Class: {record.class}</div>}
                  </td>
                  <td className="p-4">
                    <div className="text-sm text-slate-600">{new Date(record.date).toLocaleDateString()}</div>
                  </td>
                  <td className="p-4">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
                      ...
                      {record.status === 'Present' && <LuCheck size={14} />}
                      {record.status === 'Absent' && <LuX size={14} />}
                      {record.status}
                    </span>
                  </td>
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button 
                        onClick={() => { setFormData(record); setShowModal(true); }}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-fade-in-up">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
              <h2 className="text-xl font-bold text-slate-900">{formData.id ? 'Edit' : 'Mark'} Attendance</h2>
